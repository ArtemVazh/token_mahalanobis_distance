...
class TokenKNN(Estimator):
    ...
    def __str__(self):
        ...
        return f"TokenKNN_{self.embeddings_type}{hidden_layer} ({self.aggregation}, {self.metric_name}, {self.metric_thr})"
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        if not self.is_fitted:
            ...
            self.knn = NearestNeighbors(n_neighbors=10, algorithm='brute')
        ...
        dists = np.max(self.knn.kneighbors(embeddings)[0], axis=1)
...
class TokenMahalanobisDistanceClaim(Estimator):
    def __init__(
        ...
        self.hidden_layer = hidden_layer
        if self.hidden_layer == -1:
            super().__init__(["token_embeddings", "train_token_embeddings", "train_greedy_tokens", "train_target_texts", "claims", "train_claims"], "claim")
        else:
            super().__init__([f"token_embeddings_{self.hidden_layer}", f"train_token_embeddings_{self.hidden_layer}", "train_greedy_tokens", "train_target_texts", "claims", "train_claims"], "sequence")
        self.centroid = None
        self.sigma_inv = None
        self.parameters_path = parameters_path
        self.embeddings_type = embeddings_type
    ...
    def __str__(self):
        hidden_layer = "" if self.hidden_layer==-1 else f"_{self.hidden_layer}"
        return f"TokenMahalanobisDistance_{self.embeddings_type}{hidden_layer} ({self.aggregation}, {self.metric_thr})"

...
class StableLinReg:
    ...
    def fit(self, X, y):
        kfolds = KFold(n_splits=self.k_folds, shuffle=True, random_state=42)
        for i, (train_index, val_index) in enumerate(kfolds.split(X)):
            X_train, y_train = X[train_index], y[train_index]
            X_val, y_val = X[val_index], y[val_index]
            model = Ridge(positive=self.positive)
            model.fit(X_train, y_train)
            self.models.append(model)     
            self.weights[i] = get_prr(model.predict(X_val), 1 - y_val)
            print(f"fold {i}:", y_train.mean(), y_val.mean())
...
class StableTokenMahalanobisDistance(Estimator):
    def __init__(
        self,
    ...
    def __str__(self):
        ...
        msp = "+MSP" if self.add_msp else ""
        return f"Stable{self.ue}Distance_{self.embeddings_type}{msp}{hidden_layers} ({self.aggregation}, {self.metric_name}, {self.metric_md_name}, {self.metric_thr}, {positive}, {self.norm}, {tgt_norm}, {remove_corr})"

    def _fit_dev_md(self, train_idx, dev_idx, stats, layer=-1):
    ...
    def __call__(self, stats: Dict[str, np.ndarray]) -> np.ndarray:
        if not self.is_fitted: 
            ...
            if self.add_msp:
                self.weights = np.zeros(len(self.hidden_layers) + 2)
            else:
                self.weights = np.zeros(len(self.hidden_layers))
                
            self.max_md = np.zeros(self.weights.shape[-1])
            self.min_md = np.zeros(self.weights.shape[-1])
                
            self.max_md.fill(-np.inf)
            self.min_md.fill(np.inf)
            self.models = []
            # self.pcas = []
            for fold, (train_idx, dev_idx) in enumerate(kfolds.split(self.train_seq_metrics)):
                fold_mds = []
                for layer in tqdm(self.hidden_layers):
                    md = self._fit_dev_md(train_idx, dev_idx, stats, layer=layer)
                    k = 0
                    token_mean_md = []
                    for tokens in [train_greedy_tokens[k] for k in dev_idx]:
                        dists_i = md[k:k+len(tokens)]
                        k += len(tokens)
                        token_mean_md.append(np.mean(dists_i))
                    fold_mds.append(token_mean_md)                
                fold_mds = np.array(fold_mds).T
                if self.add_msp:
                    ...
                    fold_mds = np.hstack([fold_mds, msp[:, None], ent[:, None]])
                # np.save(f"./workdir/data/{self.ue}_scores_fold={fold}.npy", fold_mds)
                # np.save(f"./workdir/data/{self.ue}_metrics_fold={fold}.npy", self.train_seq_metrics[dev_idx])
                fold_mds[np.isnan(fold_mds)] = 0
                # pca = PCA(n_components=n_components)
                # fold_mds = pca.fit_transform(fold_mds)
                # self.pcas.append(pca)
                
                self.max_md = np.max([self.max_md, fold_mds.max(axis=0)], axis=0)
                self.min_md = np.min([self.min_md, fold_mds.min(axis=0)], axis=0)
                
                scores = []
                for j in range(fold_mds.shape[-1]):
                    scores.append(get_prr(fold_mds[:, j], self.train_seq_metrics[dev_idx]))
                self.weights += np.array(scores) / n_folds
                
                print(f"Weights {fold}: {scores}")
            
            if self.positive:
                self.weights[self.weights < 0] = 0
            self.weights /= np.abs(self.weights).sum()
            print(f"Weights: {self.weights}")
            print(self.max_md, self.min_md)
            self.is_fitted = True
        ...
        if self.add_msp:
            ...
            eval_dists = np.hstack([eval_dists, msp[:, None], ent[:, None]])

        eval_dists[np.isnan(eval_dists)] = 0
        # eval_dists = np.mean([pca.transform(eval_dists) for pca in self.pcas], axis=0)
        eval_dists = (eval_dists - self.min_md[None, :]) / (self.max_md - self.min_md)[None, :]
        # eval_dists = self.scaler.transform(eval_dists)
        ues = eval_dists @ self.weights
        # ues = np.mean([pca.transform(eval_dists) @ self.weights[i] for i, pca in enumerate(self.pcas)], axis=0)
        return ues
